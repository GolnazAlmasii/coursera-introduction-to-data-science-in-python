...
def names():
    ...
    if len(names) == 4:
        return(names)
        #return ("There are four names in the simple_string")    
...
def grades():
    ...
    with open ("assets/grades.txt", "r") as file:
        gradesfile = file.read()
        grades = re.findall("[\w]*\s[\w]*: B", gradesfile)
        for name in range(len(grades)):
            grades[name] = grades[name].rstrip(": B")
    #print(grades)
    if len (grades) == 16:
        return(grades)
grades()
